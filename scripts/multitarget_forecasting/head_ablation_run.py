#!/usr/bin/env python3
"""Run the all-station target-specific forecasting-head ablation."""
...
from scripts.multitarget_forecasting.head_ablation_model import train_head_ablation
...
def parse_variants(value: str) -> tuple[str, ...]:
    variants = _csv(value)
    if not variants:
        raise ValueError("至少需要一种预测头变体。")
    unknown = set(variants).difference(config.VARIANTS)
    if unknown:
        raise ValueError(f"未知预测头变体: {sorted(unknown)}")
    return variants
...
def frozen_identity() -> dict[str, object]:
    paths = (
        ...
        Path("scripts/multitarget_forecasting/model.py"),
...
def expected_metadata(
    station: str, variant: str, seed: int
) -> dict[str, object]:
    import torch

    return {
        "experiment": config.EXPERIMENT_ID,
        "kind": "joint_five_target_head_ablation_validation_prediction",
        "station": station,
        "variant": variant,
    ...
    for station in selected_stations:
        ...
        for variant in variants:
            for seed in seeds:
                ...
                prediction, diagnostics = train_head_ablation(
                    train,
                    validation,
                    variant=variant,
                    seed=seed,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="24小时输入下共享头与指标专属头的参数量受控消融"
    )
