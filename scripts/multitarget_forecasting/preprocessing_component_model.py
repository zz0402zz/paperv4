"""Train the two missing preprocessing-component ablations without changing A/D/E."""
...
def _prepare_inputs(
    ...
    raise ValueError(f"未知输入标准化: {scaler_kind}")


def _fit_target_scaler(values: np.ndarray, mask: np.ndarray, scaler_kind: str):
    if scaler_kind == "mean_std":
        return base_data.Standardizer.fit(values, mask)
    if scaler_kind == "median_iqr":
        return robust_data.RobustScaler.fit(values, mask)
    raise ValueError(f"未知目标标准化: {scaler_kind}")


def _scaler_center(scaler) -> np.ndarray:
    return np.asarray(
        scaler.mean if hasattr(scaler, "mean") else scaler.center,
        dtype=float,
    )


def _loss_torch(torch, prediction, target, mask, loss_kind: str):
    if loss_kind == "mse":
        return balanced_masked_mse(torch, prediction, target, mask)
    if loss_kind == "huber":
        return balanced_masked_huber(
            torch, prediction, target, mask, config.HUBER_DELTA
        )
    raise ValueError(f"未知损失函数: {loss_kind}")
...
def _loss_numpy(
    ...
    if loss_kind == "mse":
        return balanced_masked_mse_numpy(prediction, target, mask)
    if loss_kind == "huber":
        return balanced_masked_huber_numpy(
            prediction, target, mask, config.HUBER_DELTA
        )
    raise ValueError(f"未知损失函数: {loss_kind}")


def _train_epoch(torch, network, loader, optimizer, device, loss_kind: str) -> float:
    network.train()
    losses: list[float] = []
...
def _select_epoch(
    ...
    *,
    ...
    fit_sequence, fit_context, val_sequence, val_context, _ = _prepare_inputs(
        fit, internal_validation, str(spec["input_scaler"])
    )
    fit_values, fit_mask = head_ablation_model.mixed_targets(fit)
    val_values, val_mask = head_ablation_model.mixed_targets(internal_validation)
...
def train_component_ablation(
    ...
    *,
    ...
    if variant not in config.TRAIN_VARIANTS:
        raise ValueError(f"未知预处理组件变体: {variant}")
    spec = config.TRAIN_VARIANT_SPECS[variant]
    ...
    train_sequence, train_context, val_sequence, val_context, input_scalers = (
        _prepare_inputs(train, validation, str(spec["input_scaler"]))
    )
    train_values, train_mask = head_ablation_model.mixed_targets(train)
    ...
    prediction = head_ablation_model.to_absolute_prediction(
        predicted_mixed, validation["current"]
    )
    ...
    torch.save(
        {
            ...
            "target_center": torch.as_tensor(_scaler_center(target_scaler)),
    ...
    diagnostics = {
        ...
        "target_center": _scaler_center(target_scaler),
