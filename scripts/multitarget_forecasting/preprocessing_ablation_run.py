#!/usr/bin/env python3
"""Run validation-only preprocessing ablations for the frozen mixed-output GRU."""
...
def parse_variants(value: str) -> tuple[str, ...]:
    variants = _csv(value)
    if not variants:
        raise ValueError("至少需要一种预处理变体。")
    unknown = set(variants).difference(config.VARIANTS)
    if unknown:
        raise ValueError(f"未知预处理变体: {sorted(unknown)}")
    return variants
...
def expected_metadata(station: str, variant: str, seed: int) -> dict[str, object]:
    ...
    return {
        ...
        "kind": "joint_five_target_preprocessing_ablation_validation_prediction",
        "station": station,
        "variant": variant,
        "variant_label": config.VARIANT_LABELS[variant],
        "variant_spec": config.VARIANT_SPECS[variant],
        ...
        "scaler": "training_only_median_iqr",
    ...
    for station in selected_stations:
        ...
        for variant in variants:
            for seed in seeds:
                ...
                prediction, diagnostics = train_preprocessing_ablation(
                    train,
                    validation,
                    variant=variant,
                    seed=seed,
                ...
                arrays = {
                    ...
                    "quality_mask": np.asarray(validation["quality_y_mask"], dtype=bool),
                    "current": np.asarray(validation["current"], dtype=float),
                    "current_mask": np.asarray(validation["current_mask"], dtype=bool),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="24小时输入、混合输出GRU的稳健标准化、对数变换与质量标记消融"
    )
